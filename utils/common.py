...
def prep_input(img, gene, gene_num, is_GAN):
    gene = gene.to_dense()
    if len(gene.shape) == 4 and is_GAN:
        gene = gene.sum((1, 2))
    return img.cuda(), gene[:, :gene_num].cuda()


def post_output(img, size):
    out = F.interpolate(img, size,
                        mode='bicubic', antialias=True)
    out = prep_outim(out)
    if out.shape[1] == 2:
        out = torch.cat((torch.zeros_like(out[:, 0])[:, None],
                         out), dim=1)

    return out


def prep_inp_mat(img, gene, data, size=128):
    img = torch.from_numpy(img).float()
    if data == 'CosMx':
        img = img[:, 1:]
    img = F.interpolate(img, size, mode='bicubic',
                        antialias=True).round().clamp(0, 255)
    img = (img / 127.5 - 1)
    gene = torch.from_numpy(gene).float()
    return img.cuda(), gene.cuda()


def init_img_noise(model, row, col):
    noise = [[] for _ in range(row)]
    for layer_idx in range(model.num_layers):
        res = (layer_idx + 5) // 2
        for rid in range(row):
            sz = [col, 1, 2 ** res, 2 ** res]
            noise[rid].append(torch.randn(*sz).cuda())
    return noise


def set_img_noise(model, batch, noise=None):
    for n_idx, layer_idx in enumerate(range(model.num_layers)):
        res = (layer_idx + 5) // 2
        if noise is None:
            sz = [batch, 1, 2 ** res, 2 ** res]
            ns = torch.randn(*sz).cuda()
        else:
            ns = noise[n_idx]
        setattr(model.noises, f'noise_{layer_idx}', ns)


def prep_outim(img):
    return ((img + 1) * 127.5).round().clamp(0, 255)


def run_GAN(decoder, model, gene,
            noise=None, randomize_noise=True):
    if noise is None:
        noise = torch.randn((gene.shape[0], 512))

    if decoder == 'style2':
        sample, _ = model([noise.to(gene), gene],
                          randomize_noise=randomize_noise)
    elif decoder == 'style3':
        sample = model(noise.to(gene), gene)
    return sample


def run_GANI(decoder, model, gene, img,
             randomize_noise=True,
             shift=None):
    if decoder == 'style2':
        sample = model(img, gene,
                       randomize_noise=randomize_noise,
                       shift=shift)
    elif decoder == 'style3':
...
def run_one_enc(arg, model, avgim, input, rna):
    input = input.clone()
    ...
    # currently, it is hard to support splitted rna reprs
    assert len(avgim) == 1
    codes, style, rnas = [], [], [rna]
    for mod, avg, inp, rna in zip(model, avgim, input, rnas):
        avg = avg.unsqueeze(0)
        avg = avg.repeat(inp.shape[0], 1, 1, 1)
        # concatenate the average image to input
        avinp = torch.cat([inp, avg], dim=1)
        with torch.inference_mode():
            cod = mod.encoder(avinp, rna=rna)
            # if not arg.stat_res:

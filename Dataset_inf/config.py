...
class cfg_mouse(object):
    def __init__(self, root, subs, orgs,
        ...
        assert orgs in ('liver', 'lung', 'brain',
                        'kidney', 'colon', 'hair', 'organs')
        self.offset, self.roi_num = 0, 2
        self.raw_sz, self.vis_sz = 128, 256

        # (108670, 53991) for mouse
        self.hei, self.wid = 108670, 53991
        self.roi, self.ovlp = 2048, self.raw_sz // 2
        self.h_num = ceil(self.hei / 2048)
        self.w_num = ceil(self.wid / 2048)

        # Valid index range
        self.h_st, self.h_ed = 1, self.h_num - 1
        self.w_st, self.w_ed = 0, self.w_num - 1
        ...
        if subs == 'all':
            self.pths = list(root.glob('rna/*_rna.npz'))
            # exclude the tile with row = self.w_ed
            self.pths = [p for p in self.pths if '53184_53991' not in p.stem]
        elif subs == 'random':
            # 0.2, 0.4, 0.6, 0.8
            assert 0 < frac < 1
            if seed is not None:
                random.seed(seed)
            _pths = list(root.glob('rna/*_rna.npz'))
            _pths = [p for p in _pths if '53184_53991' not in p.stem]
            num_to_retain = int(np.round(float(len(_pths)) * frac))
            self.pths = np.random.permutation(_pths)[:num_to_retain].tolist()
        elif subs == 'checkerboard':
            assert frac in (1, 12, 14, 3, 5, 8)
            if frac == 12:
                # 2048 // (2 * 2)
                self.sm_roi = 512
            elif frac == 14:
    ...
    def _prep_org(self, orgs):
        ...
        _organ = {
            ...
            'kidney': [((11, 8), (14, 5)), ('Cdh16', 'Ldhb', 'Epcam', 'Tfcp2l1'), 'left', (0, 255, 255)],
            'colon':  [((12, 12), (15, 14)), ('Cdhr5', 'Epcam', 'Hmgcs2', 'Rbp2'), 'right', (0, 0, 255)],
            'lung':   [((23, 9), (26, 7)), ('Cav1', 'Mfap4', 'Calcrl', 'Timp3'), 'left', (255, 215, 0)],
            'liver':  [((18, 8), (19, 13)), ('C3', 'Gc', 'Hp', 'Hmgcs2'), 'right', (255, 0, 255)],
            'brain':  [((45, 10), (48, 11)), ('Stmn1', 'Gap43', 'Nnat', 'Tubb3'), 'left', (0, 255, 0)],
            'hair':   [((47, 23), (48, 20)), ('Calm4', 'Sbsn', 'Anxa2', 'Bgn'), 'right', (210, 116, 25)],
        }
        if orgs in _organ:
    ...
    def _prep_arr(self, root, subs, frac):
        ...
        for h in range(self.h_st, self.h_ed):
            for w in range(self.w_st, self.w_ed):
                ...
                pth = root / 'rna' / f'{roi_nm}_rna.npz'

...
from Dataset_inf.config_brn import cfg_mouse
...
def crop_gene(args, pth, roi=2176):
    crd_all = list(map(int, pth.stem.split('_')))
    crd, crdo = crd_all[:4], crd_all[4:]
    h_st = 0 if crd[0] == crdo[0] else 192
    h_ed = 0 if crd[1] == crdo[1] else -192
    w_st = 0 if crd[2] == crdo[2] else 192
    w_ed = 0 if crd[3] == crdo[3] else -192

    gene = sparse.load_npz(str(pth))
    gn = gene[h_st:h_ed, w_st:w_ed,
              :args.gene_num]
    crds = gn.coords
    # print('len of coord', len(crds))
    if h_st == 0:
        crds[0] = crds[0] + 64
    if w_st == 0:
        crds[1] = crds[1] + 64
    gn = sparse.COO(crds, gn.data,
                    shape=(roi, roi, gn.shape[2]))
    return gn


def crop_stain(args, pth, roi=2048):
    crd_all = list(map(int, pth.stem.split('_')))
    crd, crdo = crd_all[:4], crd_all[4:]
    h_st = 0 if crd[0] == crdo[0] else 192
    w_st = 0 if crd[2] == crdo[2] else 192

    img = pyvips.Image.new_from_file(str(pth), n=-1)
    img = img.crop(w_st, h_st, roi, roi).numpy()
    img = img[:, :, None]
...
def run_raw_crop(raw_pth, out_pth, bbx_clr,
    ...
    if not out_pth.is_file():
        if isinstance(raw_pth, (Path, str)):
...
def run_one_crop(args, cfg, pth,
    ...
    if isinstance(pth, Path):
        ...
        if bbox is not None:
            ...
            if cid == 0:
                raw_pth = Path(str(pth.parent).replace(
                    'gene', 'DAPI')) / f'{pth.stem}.tif'
                raw = crop_stain(args, raw_pth)
                run_raw_crop(raw, Path(out_pth.format('CROP_D')),
...
def init_analysis(args, step=1):
    ...
    root = Path(f'Data/MERFISH/gene_{args.data_name}')
    subs, orgs, task, crop = args.task.split('_')
    # Init gene list
    # with open(str(root / 'GAN/crop/transcripts.pickle'), 'rb') as fp:
    #     gene_lst = pickle.load(fp)
    gn = pd.read_csv(f'Dataset_inf/{args.data_name[:-1]}.csv').gene
    gene_lst = gn.values.tolist()

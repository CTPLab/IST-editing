...
from Dataset_inf.InfDataset_brn import InfDataset_brn
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            ...
            if (i + 1) in (1, 1000, 2000, 4000, 5000, 10000) or (i + 1) % 50000 == 0:
                ...
                elif int(args.data) in (609882, 609889):
                    raw_sz, offset = 128, 0
                    gpths = [f'Data/MERFISH/gene_{args.data}/28672_30720_53248_55296_28416_30976_52992_55552.npz',
                             f'Data/MERFISH/gene_{args.data}/20480_22528_40960_43008_20224_22784_40704_43264.npz']

                for gid, gpth in enumerate(gpths):
                    save_pth = check_save / 'sample' / \
                        f'{gid + 1}_{str(i + 1).zfill(6)}.png'
                    gn_raw = sparse.load_npz(gpth)
                    gn_raw = gn_raw[192:-192, 192:-192]
                    run_inf_img(g_ema, glob_n, layer_n,
                                gn_raw, offset, glob_n.shape[0],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--check_save", type=str,
                        help="path to the train output")
    parser.add_argument("--stain", type=str, default='DAPI',
    ...
    args.start_iter = 0
    dataset = InfDataset_brn(f'Data/MERFISH/gene_{args.data}',
                             500, 128, 0,
                             repeat=10**4, transform=True)
    ...
    discriminator = Discriminator(
        args.size, dataset.ldim, channel_multiplier=args.channel_multiplier, img_chn=args.img_chn
    ).to(device)
    g_ema = Generator(
        args.size, gene_num, args.latent, args.kernel_size, args.n_mlp, channel_multiplier=args.channel_multiplier, img_chn=args.img_chn,
    ).to(device)
    ...
    check_parm = f'{check_save.name}_{args.gene_use}_{args.mixing}_{args.split_scheme}_{args.kernel_size}_{args.latent}_{args.proc}_{dataset.ldim}'
    if args.validation:
        check_parm = f'{check_parm}_{args.validation}'
